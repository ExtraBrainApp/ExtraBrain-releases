...
const conceptPage = (entry: ConceptEntry): MarketingPage => ({
  ...
  sections: [
    ...
    ...(entry.practice
      ? [{ title: 'Practice the tradeoffs with ExtraBrain', items: entry.practice }]
      : []),
...
const conceptEntries: ConceptEntry[] = [
  ...
  {
    slug: 'concepts/leader-election',
    term: 'Leader election',
    title: 'Leader Election in System Design - ExtraBrain',
    h1: 'What is leader election in distributed systems?',
    description: 'Leader election selects one node to coordinate work in a distributed system while handling failures, stale leaders, and split-brain risk.',
    definition: 'Leader election is the process by which a distributed group chooses one node to coordinate a task such as accepting writes, assigning work, or managing a partition. A sound design also replaces failed leaders and prevents two nodes from acting as leader for the same term.',
    details: [
      { title: 'Terms and heartbeats', body: 'Nodes attach a monotonically increasing term to leadership and use heartbeats or leases to detect when the current leader is no longer healthy.' },
      { title: 'Majority agreement', body: 'Consensus protocols require a majority vote so two isolated minorities cannot both elect authoritative leaders.' },
      { title: 'Fencing stale leaders', body: 'Fencing tokens let downstream resources reject commands from an old leader that wakes after its lease has expired.' },
      { title: 'Design tradeoff', body: 'Faster failure detection shortens failover but raises the chance of unnecessary elections during brief network delays.' },
    ],
    practice: [
      { title: 'Rehearse the failure sequence', body: 'Draw a three-node election and explain a network partition while ExtraBrain follows the visible diagram and captures your reasoning for review.' },
      { title: 'Keep sensitive designs local', body: 'The Mac-native app can use local Parakeet transcription and local session history, keeping proprietary mock scenarios on your device.' },
      { title: 'Choose the analysis path', body: 'Use compatible on-device Gemma or bring your own OpenAI, Anthropic, Claude, Codex, or compatible provider. The free core also works as a meeting copilot for peer mocks.' },
    ],
    faq: [
      { question: 'Why do distributed systems elect a leader?', answer: 'A leader provides one coordination point for ordered writes, work assignment, or partition ownership, reducing conflicting decisions.' },
      { question: 'How does a system avoid two leaders?', answer: 'It combines majority voting, numbered terms, leases, and fencing tokens so stale or isolated leaders cannot keep making authoritative changes.' },
      { question: 'What happens when the leader fails?', answer: 'Followers stop receiving heartbeats, wait through an election timeout, and begin a new election for a higher term.' },
    ],
    related: [
      { title: 'Database replication', body: 'Leaders commonly coordinate replicated writes.', href: '/concepts/database-replication/' },
      { title: 'Consistent hashing', body: 'Another way to assign distributed ownership.', href: '/concepts/consistent-hashing/' },
      { title: 'System design interviews', body: 'Practice coordination tradeoffs on Mac.', href: '/use-cases/system-design-interviews/' },
    ],
  },
  {
    slug: 'concepts/distributed-locks',
    term: 'Distributed locks',
    title: 'Distributed Locks in System Design - ExtraBrain',
    h1: 'How do distributed locks work?',
    description: 'Distributed locks coordinate exclusive work across nodes using leases, ownership tokens, and fencing to remain safe during failures.',
    definition: 'A distributed lock grants one process temporary ownership of a shared resource across multiple machines. Because processes pause and networks fail, robust locks use expiring leases, unique ownership tokens, and fencing rather than assuming a lock holder stays alive.',
    details: [
      { title: 'Lease, not forever', body: 'The lock expires unless its owner renews it, allowing the system to recover when a process crashes while holding ownership.' },
      { title: 'Compare before release', body: 'A client releases only the token it acquired, so a delayed cleanup cannot delete a newer client lock.' },
      { title: 'Fencing tokens', body: 'Each acquisition receives an increasing number that storage or workers use to reject stale operations from former owners.' },
      { title: 'Prefer less coordination', body: 'Idempotency, database constraints, partition ownership, or optimistic concurrency are often simpler than a cross-system lock.' },
    ],
    practice: [
      { title: 'Walk through a paused client', body: 'Use a visible timeline to explain a lease expiry and stale writer while ExtraBrain records both the diagram context and your spoken recovery plan.' },
      { title: 'Review privately on Mac', body: 'Local-first Parakeet transcription and device-held session history make it practical to rehearse lock failures without uploading every recording.' },
      { title: 'Use the model you trust', body: 'Run compatible on-device AI or bring your own provider for critique. ExtraBrain has a free core and doubles as a meeting copilot during collaborative design practice.' },
    ],
    faq: [
      { question: 'Why is a distributed lock harder than a local mutex?', answer: 'The lock service, client, or network can fail independently, and a paused client may resume after its ownership has expired.' },
      { question: 'What is a fencing token?', answer: 'It is an increasing number attached to each lock acquisition so downstream systems can reject operations from an older owner.' },
      { question: 'When should I avoid distributed locks?', answer: 'Avoid them when a unique database constraint, idempotency key, partitioned owner, or optimistic concurrency can enforce the same invariant more simply.' },
    ],
    related: [
      { title: 'Idempotency', body: 'Makes repeated work safe without exclusive ownership.', href: '/concepts/idempotency/' },
      { title: 'Database replication', body: 'Shared state changes must survive node failures.', href: '/concepts/database-replication/' },
      { title: 'System design interviews', body: 'Practice failure and coordination scenarios.', href: '/use-cases/system-design-interviews/' },
    ],
  },
  {
    slug: 'concepts/quorum-reads-and-writes',
    term: 'Quorum reads and writes',
    title: 'Quorum Reads and Writes - ExtraBrain',
    h1: 'What are quorum reads and writes?',
    description: 'Quorum reads and writes use overlapping replica majorities to balance consistency, availability, and latency in distributed storage.',
    definition: 'Quorum reads and writes require responses from a chosen number of replicas before an operation succeeds. With N replicas, a design often chooses read quorum R and write quorum W so R plus W exceeds N, making the read and write sets overlap.',
    details: [
      { title: 'The overlap rule', body: 'When R plus W is greater than N, every successful read contacts at least one replica that acknowledged the latest successful write.' },
      { title: 'Tunable consistency', body: 'A larger write quorum strengthens write durability, while a larger read quorum can detect newer versions at the cost of latency and availability.' },
      { title: 'Conflicting versions', body: 'Quorum overlap does not solve concurrent writes by itself, so systems still need versions, timestamps, vector clocks, or application-level merging.' },
      { title: 'Failure budget', body: 'An operation remains available only when enough replicas are reachable to satisfy its chosen quorum.' },
    ],
    practice: [
      { title: 'Calculate the quorum live', body: 'Sketch N, R, and W values and explain a failed replica while ExtraBrain tracks the diagram and transcribes the consistency argument.' },
      { title: 'Keep architecture practice private', body: 'The Mac-native workflow supports local transcription and local history, which limits how much design-session data must leave the machine.' },
      { title: 'Compare with your chosen AI', body: 'Ask compatible on-device Gemma or a provider you control to challenge the tradeoff. The free core can capture both solo rehearsal and peer meetings.' },
    ],
    faq: [
      { question: 'Why should R plus W be greater than N?', answer: 'It forces the read set and latest successful write set to overlap at one or more replicas.' },
      { question: 'Do quorums guarantee linearizable reads?', answer: 'Not automatically. Concurrent writes, sloppy quorums, stale version metadata, and repair timing can still require stronger coordination.' },
      { question: 'How do quorum sizes affect availability?', answer: 'Larger quorums need more reachable replicas, so they can improve consistency while reducing availability and increasing latency.' },
    ],
    related: [
      { title: 'Eventual consistency', body: 'How replicas converge when reads may be stale.', href: '/concepts/eventual-consistency/' },
      { title: 'Database replication', body: 'The replica topology behind quorum operations.', href: '/concepts/database-replication/' },
      { title: 'CAP theorem', body: 'The partition tradeoff quorums navigate.', href: '/concepts/cap-theorem/' },
    ],
  },
  {
    slug: 'concepts/service-discovery',
    term: 'Service discovery',
    title: 'Service Discovery in System Design - ExtraBrain',
    h1: 'What is service discovery?',
    description: 'Service discovery lets changing service instances register, find one another, pass health checks, and receive traffic without fixed addresses.',
    definition: 'Service discovery maps a logical service name to healthy network endpoints. It lets instances scale, restart, and move without requiring callers to keep a hard-coded list of addresses.',
    details: [
      { title: 'Registration', body: 'Instances register themselves or are observed by the platform, publishing addresses and metadata to a registry.' },
      { title: 'Client or server lookup', body: 'Clients can query the registry directly, or a load balancer and DNS layer can resolve the service on their behalf.' },
      { title: 'Health and expiry', body: 'Registrations need heartbeats, health checks, or short DNS lifetimes so dead instances stop receiving new traffic.' },
      { title: 'Control-plane tradeoff', body: 'A registry improves dynamism but becomes critical infrastructure that must be replicated, secured, and tolerant of stale data.' },
    ],
    practice: [
      { title: 'Trace an instance replacement', body: 'Explain registration, health failure, and lookup refresh on a service diagram while ExtraBrain keeps the visible topology tied to your narration.' },
      { title: 'Use a local-first review loop', body: 'Mac-native local transcription and local session history let you revisit a proprietary service map without creating a hosted recording by default.' },
      { title: 'Bring the reviewer you prefer', body: 'Use on-device AI where compatible or connect your own provider to test edge cases. The free core meeting copilot also records peer design feedback.' },
    ],
    faq: [
      { question: 'What problem does service discovery solve?', answer: 'It lets callers find healthy instances even as addresses change because of scaling, deployments, or failures.' },
      { question: 'What is the difference between client-side and server-side discovery?', answer: 'Client-side discovery lets each caller select an instance; server-side discovery delegates lookup and selection to a proxy or load balancer.' },
      { question: 'Can DNS provide service discovery?', answer: 'Yes. DNS-based discovery is simple and widely supported, but caching and record-update delays must match the expected failure speed.' },
    ],
    related: [
      { title: 'Load balancing', body: 'Selects among the discovered endpoints.', href: '/concepts/load-balancing/' },
      { title: 'Microservices', body: 'Dynamic service fleets need discovery.', href: '/concepts/microservices/' },
      { title: 'System design interviews', body: 'Practice routing and failure tradeoffs.', href: '/use-cases/system-design-interviews/' },
    ],
  },
  {
    slug: 'concepts/health-checks',
    term: 'Health checks',
    title: 'Health Checks in System Design - ExtraBrain',
    h1: 'How do health checks work in distributed systems?',
    description: 'Health checks distinguish process liveness from traffic readiness so orchestrators and load balancers can recover services safely.',
    definition: 'Health checks expose whether a process is alive, ready for traffic, and able to serve critical dependencies. Separating these signals keeps a slow startup from causing restarts and keeps an unhealthy instance out of the request path.',
    details: [
      { title: 'Liveness', body: 'A liveness probe asks whether the process is stuck and should be restarted, not whether every dependency is currently healthy.' },
      { title: 'Readiness', body: 'A readiness probe controls whether an instance receives traffic, accounting for startup, draining, and temporary inability to serve.' },
      { title: 'Shallow vs deep checks', body: 'Deep checks catch dependency failures but can amplify an outage if every instance declares itself unhealthy because one shared service is down.' },
      { title: 'Timing matters', body: 'Intervals, timeouts, and failure thresholds balance quick removal against flapping during brief latency spikes.' },
    ],
    practice: [
      { title: 'Diagnose the cascading case', body: 'Draw a shared database failure and explain why deep readiness checks can remove every app node while ExtraBrain captures the causal chain.' },
      { title: 'Retain the exercise locally', body: 'Local Parakeet transcription and local session history support a private, Mac-native postmortem-style review workflow.' },
      { title: 'Test assumptions with flexible AI', body: 'Use compatible on-device Gemma or a bring-your-own provider to challenge thresholds. The free core is also useful as a meeting copilot in group mocks.' },
    ],
    faq: [
      { question: 'What is the difference between liveness and readiness?', answer: 'Liveness decides whether to restart a process, while readiness decides whether it should receive traffic.' },
      { question: 'Should a health check call every dependency?', answer: 'Usually not for liveness. Readiness may check critical capabilities, but deep dependency checks can cause a shared outage to remove otherwise useful instances.' },
      { question: 'How do you prevent health-check flapping?', answer: 'Use sensible timeouts, consecutive success and failure thresholds, startup grace periods, and gradual traffic changes.' },
    ],
    related: [
      { title: 'Load balancing', body: 'Uses health signals to route around failures.', href: '/concepts/load-balancing/' },
      { title: 'Circuit breaker', body: 'Limits calls to unhealthy dependencies.', href: '/concepts/circuit-breaker/' },
      { title: 'System design interviews', body: 'Practice production-failure scenarios.', href: '/use-cases/system-design-interviews/' },
    ],
  },
  {
    slug: 'concepts/backpressure',
    term: 'Backpressure',
    title: 'Backpressure in System Design - ExtraBrain',
    h1: 'What is backpressure in distributed systems?',
    description: 'Backpressure keeps fast producers from overwhelming slower consumers by slowing, buffering, shedding, or rejecting incoming work.',
    definition: 'Backpressure is feedback from a saturated consumer to the producers sending it work. Instead of allowing queues and memory to grow without bound, the system slows intake, limits concurrency, rejects excess requests, or degrades nonessential work.',
    details: [
      { title: 'Bound the queue', body: 'A finite queue makes overload visible and defines where the system will wait, reject, or shed work instead of failing through memory exhaustion.' },
      { title: 'Propagate the signal', body: 'Protocols can pause reads, reduce request concurrency, lower batch size, or return overload responses that callers respect.' },
      { title: 'Protect latency', body: 'Rejecting some work early can preserve useful latency for admitted requests because an unbounded queue only converts overload into long waits.' },
      { title: 'Choose the policy', body: 'Dropping newest, dropping oldest, sampling, and prioritizing each fit different workloads and business guarantees.' },
    ],
    practice: [
      { title: 'Model a traffic spike', body: 'Annotate producer rate, consumer rate, and queue depth on screen while ExtraBrain records how your admission policy changes over time.' },
      { title: 'Review operational examples privately', body: 'The local-first Mac app can transcribe with Parakeet and retain the mock in local session history rather than requiring hosted audio storage.' },
      { title: 'Stress-test the policy', body: 'Use on-device AI where compatible or a provider you bring to question drop behavior. The free core meeting copilot also captures a group capacity review.' },
    ],
    faq: [
      { question: 'What happens without backpressure?', answer: 'Queues, threads, connections, or memory keep growing until latency becomes unacceptable or the service fails.' },
      { question: 'Is rate limiting the same as backpressure?', answer: 'No. Rate limits enforce an intake policy, while backpressure is a saturation signal that can dynamically slow producers throughout a pipeline.' },
      { question: 'Why can rejection improve reliability?', answer: 'Early rejection prevents every admitted request from waiting behind an unbounded backlog, preserving capacity for useful work and recovery.' },
    ],
    related: [
      { title: 'Rate limiting', body: 'Controls admission before work enters the system.', href: '/concepts/rate-limiting/' },
      { title: 'Message queues', body: 'Buffers work but still needs a capacity policy.', href: '/concepts/message-queues/' },
      { title: 'Circuit breaker', body: 'Fails fast when a dependency is overloaded.', href: '/concepts/circuit-breaker/' },
    ],
  },
  {
    slug: 'concepts/bulkhead-pattern',
    term: 'The bulkhead pattern',
    title: 'Bulkhead Pattern in System Design - ExtraBrain',
    h1: 'What is the bulkhead pattern?',
    description: 'The bulkhead pattern isolates resources by workload so a failure or traffic surge in one area cannot exhaust an entire service.',
    definition: 'The bulkhead pattern divides a system into isolated resource pools, named after the sealed compartments in a ship. If one workload consumes its pool or fails, unrelated workloads retain threads, connections, memory, or instances.',
    details: [
      { title: 'Resource isolation', body: 'Separate thread pools, connection pools, queues, or service instances prevent one caller or dependency from consuming the shared capacity.' },
      { title: 'Failure containment', body: 'An overloaded reporting job can fail within its partition while checkout or another critical path continues serving requests.' },
      { title: 'Sizing tradeoff', body: 'Too little isolation allows noisy-neighbor failures, while too much strands spare capacity in a pool that other work cannot borrow.' },
      { title: 'Priority boundaries', body: 'Bulkheads work well when traffic classes have different importance, latency goals, or failure characteristics.' },
    ],
    practice: [
      { title: 'Partition a shared pool', body: 'Use ExtraBrain screen context to rehearse splitting threads among interactive and batch traffic, then inspect the transcript for an explicit sizing rationale.' },
      { title: 'Keep workload maps on the Mac', body: 'Local transcription and local session history support private review of architecture examples that may resemble internal systems.' },
      { title: 'Challenge the capacity split', body: 'Bring your own provider or use compatible local AI for a second opinion. ExtraBrain has a free core and can capture peer debate as a meeting copilot.' },
    ],
    faq: [
      { question: 'What failure does the bulkhead pattern prevent?', answer: 'It prevents one overloaded workload or failing dependency from consuming all shared resources and taking unrelated paths down with it.' },
      { question: 'What resources can be isolated?', answer: 'Common choices include thread pools, database connections, queues, rate limits, worker groups, and separate service deployments.' },
      { question: 'What is the cost of bulkheads?', answer: 'Isolation adds configuration and can strand capacity when one pool is idle while another is saturated.' },
    ],
    related: [
      { title: 'Circuit breaker', body: 'Stops calls to a failing dependency.', href: '/concepts/circuit-breaker/' },
      { title: 'Backpressure', body: 'Prevents isolated pools from growing without bound.', href: '/concepts/backpressure/' },
      { title: 'Microservices', body: 'Service boundaries can become failure boundaries.', href: '/concepts/microservices/' },
    ],
  },
  {
    slug: 'concepts/retry-with-exponential-backoff',
    term: 'Retry with exponential backoff',
    title: 'Retry with Exponential Backoff - ExtraBrain',
    h1: 'How does exponential backoff work?',
    description: 'Exponential backoff spaces retries farther apart and adds jitter to avoid synchronized retry storms during transient failures.',
    definition: 'Exponential backoff increases the delay after each failed attempt, commonly doubling it up to a cap. Adding random jitter spreads callers across time so they do not retry together and prolong an outage.',
    details: [
      { title: 'Retry only transient failures', body: 'Timeouts, connection resets, and overload responses may recover; validation errors and permanent authorization failures generally will not.' },
      { title: 'Add jitter', body: 'Randomizing each delay prevents thousands of clients from waking on the same schedule and creating another traffic spike.' },
      { title: 'Set a budget', body: 'Limit attempts, total elapsed time, and per-attempt timeout so retries do not consume the full user deadline.' },
      { title: 'Require safe repetition', body: 'Retrying a write needs idempotency or deduplication because the first attempt may have succeeded even when its response was lost.' },
    ],
    practice: [
      { title: 'Build the retry timeline', body: 'Show attempts, deadlines, and jitter on a diagram while ExtraBrain captures whether your policy respects the caller latency budget.' },
      { title: 'Analyze incidents locally', body: 'Mac-native Parakeet transcription and local history make private rehearsal possible when the scenario comes from a sensitive postmortem.' },
      { title: 'Use flexible critique', body: 'Run compatible on-device Gemma or connect a provider you control to probe edge cases. The free core meeting mode works for team-based mock reviews.' },
    ],
    faq: [
      { question: 'Why add jitter to exponential backoff?', answer: 'Without jitter, clients that failed together tend to retry together, creating synchronized traffic spikes called a thundering herd.' },
      { question: 'Which errors should be retried?', answer: 'Retry errors that are plausibly transient, such as timeouts or explicit overload responses, and avoid retrying permanent client errors.' },
      { question: 'Can retries duplicate writes?', answer: 'Yes. A response can be lost after a write succeeds, so retryable write APIs need idempotency keys or another deduplication mechanism.' },
    ],
    related: [
      { title: 'Idempotency', body: 'Makes repeated write attempts safe.', href: '/concepts/idempotency/' },
      { title: 'Circuit breaker', body: 'Stops retries when a dependency is persistently unhealthy.', href: '/concepts/circuit-breaker/' },
      { title: 'Backpressure', body: 'Signals callers to reduce offered load.', href: '/concepts/backpressure/' },
    ],
  },
  {
    slug: 'concepts/saga-pattern',
    term: 'The saga pattern',
    title: 'Saga Pattern for Distributed Transactions - ExtraBrain',
    h1: 'What is the saga pattern?',
    description: 'The saga pattern coordinates a distributed business transaction as local steps with compensating actions for partial failure.',
    definition: 'The saga pattern breaks a cross-service transaction into a sequence of local transactions. Each completed step has a compensating action that semantically reverses it when a later step fails, avoiding one long database lock across services.',
    details: [
      { title: 'Orchestration', body: 'A central coordinator tells each participant what to do next and records progress, making the workflow explicit and observable.' },
      { title: 'Choreography', body: 'Services react to events without a central controller, reducing central coupling but making the full flow harder to follow.' },
      { title: 'Compensation is business logic', body: 'A compensating action may refund or release rather than literally undo, and it must be safe to retry.' },
      { title: 'Intermediate states', body: 'Other requests can observe a saga between steps, so the domain model must represent pending, confirmed, failed, and compensated states.' },
    ],
    practice: [
      { title: 'Rehearse a partial failure', body: 'Map order, payment, and inventory steps on screen while ExtraBrain transcribes how each compensation preserves the business invariant.' },
      { title: 'Keep the domain example private', body: 'The local-first Mac app offers local transcription, device-held history, and on-device AI options for sensitive workflow practice.' },
      { title: 'Compare coordination styles', body: 'Bring your own provider to challenge orchestration versus choreography. The free core can also serve as a meeting copilot for a peer design round.' },
    ],
    faq: [
      { question: 'Why use a saga instead of a distributed transaction?', answer: 'A saga avoids holding locks across service databases and keeps services independently deployable, at the cost of more application-level recovery logic.' },
      { question: 'What is a compensating transaction?', answer: 'It is a domain action that offsets a completed step, such as refunding a payment or releasing reserved inventory.' },
      { question: 'What is the difference between orchestration and choreography?', answer: 'Orchestration uses a central workflow coordinator, while choreography lets services advance the saga by reacting to events.' },
    ],
    related: [
      { title: 'Eventual consistency', body: 'Sagas move through visible intermediate states.', href: '/concepts/eventual-consistency/' },
      { title: 'Message queues', body: 'Carry reliable commands and events between steps.', href: '/concepts/message-queues/' },
      { title: 'Idempotency', body: 'Keeps steps and compensation safe to retry.', href: '/concepts/idempotency/' },
    ],
  },
  {
    slug: 'concepts/transactional-outbox',
    term: 'The transactional outbox pattern',
    title: 'Transactional Outbox Pattern - ExtraBrain',
    h1: 'What is the transactional outbox pattern?',
    description: 'A transactional outbox stores a state change and its event together, then publishes the event asynchronously without a dual-write gap.',
    definition: 'The transactional outbox pattern writes domain data and an event record to the same database transaction. A separate relay publishes pending records to a broker, avoiding the inconsistency that occurs when a database write succeeds but a direct message publish fails.',
    details: [
      { title: 'One atomic commit', body: 'The application updates its business tables and inserts the outbox row in one local transaction, so both happen or neither happens.' },
      { title: 'Asynchronous relay', body: 'A polling publisher or change-data-capture process reads committed outbox rows and sends them to the message broker.' },
      { title: 'At-least-once delivery', body: 'A relay can crash after publishing but before marking a row complete, so consumers must handle duplicate events.' },
      { title: 'Ordering scope', body: 'Sequence numbers or aggregate keys preserve the order that matters without requiring one total order for the entire system.' },
    ],
    practice: [
      { title: 'Expose the dual-write failure', body: 'Draw the database and broker sequence while ExtraBrain captures why a crash between them loses an event and how the outbox closes that gap.' },
      { title: 'Review implementation choices locally', body: 'Mac-native local transcription and local session history help keep proprietary event schemas and failure cases on the device.' },
      { title: 'Evaluate relay options', body: 'Use compatible local AI or a bring-your-own provider to compare polling with CDC. The free core meeting copilot can record a collaborative design critique.' },
    ],
    faq: [
      { question: 'What problem does the transactional outbox solve?', answer: 'It prevents a database state change from committing without the event that other services need to learn about that change.' },
      { question: 'Does an outbox provide exactly-once delivery?', answer: 'Usually not. It reliably records intent but relays commonly publish at least once, so consumers still need idempotent handling.' },
      { question: 'How are outbox rows published?', answer: 'A worker can poll the table, or a change-data-capture connector can stream committed rows from the database log.' },
    ],
    related: [
      { title: 'Change data capture', body: 'Can stream committed outbox rows.', href: '/concepts/change-data-capture/' },
      { title: 'Message queues', body: 'Deliver the published event to consumers.', href: '/concepts/message-queues/' },
      { title: 'Idempotency', body: 'Protects consumers from duplicate delivery.', href: '/concepts/idempotency/' },
    ],
  },
  {
    slug: 'concepts/change-data-capture',
    term: 'Change data capture',
    title: 'Change Data Capture in System Design - ExtraBrain',
    h1: 'What is change data capture?',
    description: 'Change data capture streams committed database changes to downstream systems without repeated full-table scans or fragile dual writes.',
    definition: 'Change data capture, or CDC, detects committed inserts, updates, and deletes and turns them into an ordered change stream. Log-based CDC reads the database transaction log, allowing downstream indexes, caches, and analytics systems to update without modifying the source write path.',
    details: [
      { title: 'Log-based capture', body: 'A connector reads the durable transaction log, so it observes the same committed order the database uses for recovery and replication.' },
      { title: 'Initial snapshot', body: 'A new consumer often needs a consistent snapshot of existing rows followed by changes after the snapshot position.' },
      { title: 'Schema evolution', body: 'Change records need versioned schemas and compatibility rules so adding or changing columns does not break consumers.' },
      { title: 'Delivery semantics', body: 'Connectors usually deliver at least once, so consumers track offsets and apply changes idempotently.' },
    ],
    practice: [
      { title: 'Trace snapshot to stream', body: 'Draw the handoff from an initial table snapshot to the live log while ExtraBrain captures how you avoid gaps and duplicates.' },
      { title: 'Protect production-shaped examples', body: 'Local-first transcription and local Mac history let you rehearse with realistic schemas while limiting external data exposure.' },
      { title: 'Compare downstream designs', body: 'Use compatible on-device AI or your own provider to test replay and schema choices. The free core meeting copilot can document a peer review.' },
    ],
    faq: [
      { question: 'Why use CDC instead of polling a table?', answer: 'CDC can capture every committed change in order with less repeated query load and without relying on an easily missed updated-at window.' },
      { question: 'What is log-based CDC?', answer: 'It reads the database transaction log rather than adding triggers or repeatedly scanning source tables.' },
      { question: 'How does a new CDC consumer start?', answer: 'It takes or receives a consistent snapshot, records the corresponding log position, and then consumes changes from that point.' },
    ],
    related: [
      { title: 'Transactional outbox', body: 'Pairs domain updates with events atomically.', href: '/concepts/transactional-outbox/' },
      { title: 'Database replication', body: 'Also consumes ordered database changes.', href: '/concepts/database-replication/' },
      { title: 'Message queues', body: 'Distribute captured changes to consumers.', href: '/concepts/message-queues/' },
    ],
  },
  {
    slug: 'concepts/event-sourcing',
    term: 'Event sourcing',
    title: 'Event Sourcing in System Design - ExtraBrain',
    h1: 'What is event sourcing?',
    description: 'Event sourcing stores immutable domain events as the source of truth and rebuilds current state by replaying them into projections.',
    definition: 'Event sourcing persists the sequence of domain events that changed an entity instead of only its latest state. Current views are projections derived from that history, which creates a strong audit trail and replay capability but moves complexity into event design and evolution.',
    details: [
      { title: 'Events are facts', body: 'An event records something that already happened, such as OrderPlaced, and should not be edited after it becomes part of history.' },
      { title: 'Projection', body: 'A read model folds events into a view optimized for queries and can be rebuilt when its logic changes.' },
      { title: 'Snapshots', body: 'Periodic snapshots reduce the cost of rebuilding long-lived aggregates without replacing the authoritative event history.' },
      { title: 'Event evolution', body: 'Old events remain replayable, so schemas need compatibility, upcasting, or version-aware projection code.' },
    ],
    practice: [
      { title: 'Rebuild an aggregate aloud', body: 'Step through several domain events and a failed projection while ExtraBrain relates the visible stream to your recovery explanation.' },
      { title: 'Keep domain models local', body: 'The Mac-native app supports local Parakeet transcription and local session history for private rehearsal of business-specific examples.' },
      { title: 'Test whether the complexity pays off', body: 'Ask on-device AI where compatible or a provider you control to challenge the fit. Use the free core as a meeting copilot during event-modeling mocks.' },
    ],
    faq: [
      { question: 'Is event sourcing the same as an audit log?', answer: 'No. An audit log records activity, while an event-sourced system treats the event history as the authoritative data used to derive current state.' },
      { question: 'Why are snapshots used in event sourcing?', answer: 'Snapshots store a derived state at a known event position so an aggregate does not need to replay its entire history on every load.' },
      { question: 'When should event sourcing be avoided?', answer: 'Avoid it when ordinary state storage meets the audit and workflow needs, because event evolution, replay, projections, and operational debugging add substantial complexity.' },
    ],
    related: [
      { title: 'CQRS', body: 'Often separates event-backed writes from read projections.', href: '/concepts/cqrs/' },
      { title: 'Eventual consistency', body: 'Projections may lag behind accepted commands.', href: '/concepts/eventual-consistency/' },
      { title: 'Message queues', body: 'Can distribute events to projection workers.', href: '/concepts/message-queues/' },
    ],
  },
  {
    slug: 'concepts/cqrs',
    term: 'CQRS',
    title: 'CQRS in System Design - ExtraBrain',
    h1: 'What is CQRS?',
    description: 'CQRS separates command and query models so writes can enforce domain rules while reads use independently optimized projections.',
    definition: 'Command Query Responsibility Segregation, or CQRS, separates the model that accepts state-changing commands from the model that serves reads. The two sides may share a database or use independently shaped storage, depending on how different their workloads are.',
    details: [
      { title: 'Command side', body: 'Commands express intent and pass through validation and domain rules before producing a state change.' },
      { title: 'Query side', body: 'Read models are shaped for the exact access pattern, avoiding complex joins or domain objects in high-volume query paths.' },
      { title: 'Synchronization', body: 'When read models update asynchronously, clients must tolerate a delay between an accepted command and the visible query result.' },
      { title: 'Use the smallest version', body: 'Separate code paths can provide clarity without immediately requiring separate databases, event sourcing, or a distributed deployment.' },
    ],
    practice: [
      { title: 'Separate the workloads', body: 'Sketch command validation and a denormalized read view while ExtraBrain records why the split is justified by access patterns rather than fashion.' },
      { title: 'Review privately after the mock', body: 'Local transcription and local history provide a Mac-native way to revisit sensitive data-model discussion without default cloud retention.' },
      { title: 'Pressure-test the boundary', body: 'Use compatible local Gemma or bring your own AI provider to probe synchronization. The free core meeting copilot can capture reviewer feedback.' },
    ],
    faq: [
      { question: 'Does CQRS require separate databases?', answer: 'No. It requires distinct command and query responsibilities; separate storage is an optional optimization when the workloads justify it.' },
      { question: 'Does CQRS require event sourcing?', answer: 'No. CQRS and event sourcing often appear together, but commands can update ordinary state while a separate query model serves reads.' },
      { question: 'What is the main cost of CQRS?', answer: 'It introduces more models and synchronization paths, especially when read projections update asynchronously and can temporarily lag.' },
    ],
    related: [
      { title: 'Event sourcing', body: 'Can provide the history that builds read models.', href: '/concepts/event-sourcing/' },
      { title: 'Change data capture', body: 'Can update query stores from database changes.', href: '/concepts/change-data-capture/' },
      { title: 'Eventual consistency', body: 'Explains temporary read-model lag.', href: '/concepts/eventual-consistency/' },
    ],
  },
  {
    slug: 'concepts/write-ahead-log',
    term: 'Write-ahead logging',
    title: 'Write-Ahead Logging in Databases - ExtraBrain',
    h1: 'What is a write-ahead log?',
    description: 'A write-ahead log records durable changes before data pages are modified, enabling crash recovery, replication, and efficient batched writes.',
    definition: 'A write-ahead log, or WAL, appends a description of each change to durable storage before the database updates its main data pages. After a crash, the database replays committed records and ignores incomplete work to restore a consistent state.',
    details: [
      { title: 'Log before data', body: 'The database must flush the relevant log record before acknowledging a commit whose data pages are not yet durable.' },
      { title: 'Sequential I/O', body: 'Appending to a log is more efficient than synchronously writing many scattered data pages for every transaction.' },
      { title: 'Crash recovery', body: 'Checkpoints limit how far recovery must scan, while redo and undo rules reconstruct committed state and discard incomplete transactions.' },
      { title: 'Replication source', body: 'Replicas can consume an ordered WAL stream, though physical and logical formats expose different levels of detail.' },
    ],
    practice: [
      { title: 'Narrate a crash boundary', body: 'Mark log flush, commit acknowledgement, and page write on a timeline while ExtraBrain captures exactly which records recovery must replay.' },
      { title: 'Keep database internals practice local', body: 'Mac-native local transcription and local session history support private review without sending every technical rehearsal to a hosted service.' },
      { title: 'Choose how to analyze it', body: 'Use compatible on-device AI or a provider you bring for critique. The free core also acts as a meeting copilot in database study sessions.' },
    ],
    faq: [
      { question: 'Why must the log be written before data pages?', answer: 'If a crash happens before dirty pages reach disk, the durable log still contains enough information to replay committed changes.' },
      { question: 'What is a database checkpoint?', answer: 'A checkpoint records a recovery position and coordinates page flushing so startup does not need to scan the log from the beginning.' },
      { question: 'Is a WAL the same as an event log?', answer: 'No. A WAL is an internal recovery structure, while a domain event log captures business facts intended for application-level consumption.' },
    ],
    related: [
      { title: 'ACID properties', body: 'WAL supports atomicity and durability.', href: '/concepts/acid-properties/' },
      { title: 'Database replication', body: 'Replicas often consume ordered log records.', href: '/concepts/database-replication/' },
      { title: 'Change data capture', body: 'Can translate transaction logs into row changes.', href: '/concepts/change-data-capture/' },
    ],
  },
  {
    slug: 'concepts/log-structured-merge-tree',
    term: 'Log-structured merge trees',
    title: 'Log-Structured Merge Trees - ExtraBrain',
    h1: 'What is a log-structured merge tree?',
    description: 'An LSM tree turns random writes into sequential batches across memory and sorted disk levels, trading read and compaction work for write throughput.',
    definition: 'A log-structured merge tree, or LSM tree, buffers writes in memory, flushes them as sorted immutable files, and compacts those files into larger sorted levels. It favors high write throughput but must manage read amplification, write amplification, and compaction cost.',
    details: [
      { title: 'Memtable and flush', body: 'Writes enter an ordered in-memory table and a recovery log, then flush to disk as an immutable sorted string table.' },
      { title: 'Compaction', body: 'Background work merges overlapping files, discards overwritten or deleted values, and maintains the chosen level structure.' },
      { title: 'Read path', body: 'A lookup may check memory and several disk files, so indexes and Bloom filters skip files that cannot contain the key.' },
      { title: 'Amplification tradeoffs', body: 'Compaction strategy balances extra bytes read, extra bytes written, temporary space, and lookup latency.' },
    ],
    practice: [
      { title: 'Trace one key through levels', body: 'Show updates, tombstones, and compaction on screen while ExtraBrain records how the latest visible value is found.' },
      { title: 'Study privately on a Mac', body: 'Local Parakeet transcription, local history, and compatible on-device AI reduce the need to upload every database-internals session.' },
      { title: 'Compare storage choices', body: 'Bring your own provider to challenge LSM versus B-tree tradeoffs. The free core meeting copilot can capture a peer walkthrough.' },
    ],
    faq: [
      { question: 'Why are LSM trees good for writes?', answer: 'They batch many updates into sequential appends and background merges instead of performing a random in-place disk update for every write.' },
      { question: 'What is compaction?', answer: 'Compaction merges sorted files, removes obsolete versions and tombstones, and reorganizes data into the target level structure.' },
      { question: 'What are read and write amplification?', answer: 'Read amplification is extra data checked per lookup, while write amplification is the additional rewriting caused by repeated compaction.' },
    ],
    related: [
      { title: 'Bloom filters', body: 'Skip sorted files that cannot contain a key.', href: '/concepts/bloom-filters/' },
      { title: 'Write-ahead log', body: 'Protects in-memory writes before a flush.', href: '/concepts/write-ahead-log/' },
      { title: 'Database indexing', body: 'Compares storage structures for lookup workloads.', href: '/concepts/database-indexing/' },
    ],
  },
  {
    slug: 'concepts/b-tree-vs-lsm-tree',
    term: 'B-trees vs LSM trees',
    title: 'B-Trees vs LSM Trees - ExtraBrain',
    h1: 'How do B-trees and LSM trees differ?',
    description: 'B-trees update balanced pages in place; LSM trees batch writes into sorted runs. The right choice depends on reads, writes, and storage behavior.',
    definition: 'B-trees keep keys in balanced, page-oriented nodes and update those pages as data changes. LSM trees buffer writes and merge sorted files later. B-trees usually offer predictable point reads, while LSM trees favor sustained write throughput.',
    details: [
      { title: 'Write path', body: 'A B-tree locates and changes a page, while an LSM tree appends to a log, updates memory, and shifts sorting work into background compaction.' },
      { title: 'Read path', body: 'A B-tree follows a short page path to the target, while an LSM lookup may consult memory and several sorted files.' },
      { title: 'Amplification', body: 'B-trees can amplify random I/O and page updates; LSM trees can amplify reads and rewrite data repeatedly during compaction.' },
      { title: 'Workload fit', body: 'Use actual point-read, range-scan, write-rate, storage, and latency requirements instead of assuming one structure is universally faster.' },
    ],
    practice: [
      { title: 'Compare one workload', body: 'Put a read-heavy index and write-heavy event store side by side while ExtraBrain captures the storage-engine reasoning tied to each diagram.' },
      { title: 'Retain the review on device', body: 'The local-first Mac workflow offers local transcription and local session history for private study of production-shaped workload numbers.' },
      { title: 'Ask a model you control', body: 'Use compatible on-device Gemma or your own AI provider to challenge assumptions. The free core meeting copilot supports peer database mocks.' },
    ],
    faq: [
      { question: 'Are B-trees always better for reads?', answer: 'They often provide more predictable point reads, but caching, Bloom filters, compaction, and workload distribution can change the result.' },
      { question: 'Why do LSM trees have high write throughput?', answer: 'They turn many small random updates into sequential log writes, memory updates, and batched background merges.' },
      { question: 'Which structure is used by relational databases?', answer: 'Many relational indexes use B-tree variants, while several write-oriented distributed and embedded stores use LSM-tree families.' },
    ],
    related: [
      { title: 'Log-structured merge trees', body: 'The full LSM write and compaction path.', href: '/concepts/log-structured-merge-tree/' },
      { title: 'Database indexing', body: 'How index structures accelerate access.', href: '/concepts/database-indexing/' },
      { title: 'Bloom filters', body: 'Reduce LSM read amplification.', href: '/concepts/bloom-filters/' },
    ],
  },
  {
    slug: 'concepts/read-replicas',
    term: 'Read replicas',
    title: 'Read Replicas in System Design - ExtraBrain',
    h1: 'How do read replicas work?',
    description: 'Read replicas copy data from a primary database to scale queries and improve resilience, with explicit lag and consistency tradeoffs.',
    definition: 'A read replica maintains a copy of primary database changes and serves queries without taking writes. Replicas add read capacity and regional proximity, but replication lag means a client may not immediately see its own successful write.',
    details: [
      { title: 'Replication stream', body: 'The primary sends an ordered change log that each replica applies, usually asynchronously to keep write latency low.' },
      { title: 'Route eligible reads', body: 'Analytics, feeds, and stale-tolerant lookups fit replicas; workflows that require the newest committed value may need the primary.' },
      { title: 'Read-your-writes', body: 'Session pinning, version tokens, or a short primary-read window can keep a user from seeing older state after an update.' },
      { title: 'Failover is separate', body: 'Promoting a replica requires checking its freshness, preventing the old primary from writing, and redirecting both writers and readers.' },
    ],
    practice: [
      { title: 'Explain a stale read', body: 'Trace a write and immediate profile read across the primary and replica while ExtraBrain records the consistency repair you choose.' },
      { title: 'Review data-flow examples locally', body: 'Mac-native local transcription and local history let database-topology practice remain private by default.' },
      { title: 'Probe routing strategies', body: 'Use compatible local AI or a bring-your-own provider to test read-your-writes options. The free core meeting mode captures peer feedback.' },
    ],
    faq: [
      { question: 'Can a read replica return stale data?', answer: 'Yes. With asynchronous replication, a replica can lag behind the primary and temporarily miss a recently committed write.' },
      { question: 'How can an application provide read-your-writes consistency?', answer: 'It can temporarily read from the primary, pin a session, or wait until a replica reaches a version token from the completed write.' },
      { question: 'Do read replicas automatically provide failover?', answer: 'No. Promotion, stale-primary fencing, writer redirection, and recovery procedures are separate design concerns.' },
    ],
    related: [
      { title: 'Database replication', body: 'Covers synchronous and asynchronous replica tradeoffs.', href: '/concepts/database-replication/' },
      { title: 'Eventual consistency', body: 'Explains why replicas can temporarily disagree.', href: '/concepts/eventual-consistency/' },
      { title: 'Leader election', body: 'Can coordinate primary selection during failover.', href: '/concepts/leader-election/' },
    ],
  },
  {
    slug: 'concepts/database-connection-pooling',
    term: 'Database connection pooling',
    title: 'Database Connection Pooling - ExtraBrain',
    h1: 'What is database connection pooling?',
    description: 'A database connection pool reuses a bounded set of established connections, reducing setup cost while protecting the database from overload.',
    definition: 'Database connection pooling keeps a reusable, bounded set of open connections between an application and its database. Borrowing an existing connection avoids repeated authentication and setup, while the pool limit prevents every request from opening its own database session.',
    details: [
      { title: 'Bound concurrency', body: 'The pool caps simultaneous database work, but callers need a finite acquisition timeout so overload does not create an endless wait.' },
      { title: 'Size the whole fleet', body: 'Per-instance pool size multiplied by the number of application instances must stay within the database connection budget.' },
      { title: 'Clean borrowed state', body: 'Transactions, session variables, temporary tables, and prepared state must be reset before a connection returns to another caller.' },
      { title: 'Observe saturation', body: 'Track active, idle, waiting, timeout, and query-duration metrics to separate pool starvation from slow database work.' },
    ],
    practice: [
      { title: 'Calculate fleet capacity', body: 'Work through instances, pool size, and database limits on screen while ExtraBrain transcribes where queueing and rejection should happen.' },
      { title: 'Keep performance cases private', body: 'Local-first transcription and local session history provide a Mac-native review path for workload details and incident-derived numbers.' },
      { title: 'Challenge the sizing model', body: 'Use on-device AI where compatible or connect your own provider for critique. The free core meeting copilot can capture a capacity-planning mock.' },
    ],
    faq: [
      { question: 'Why not open one database connection per request?', answer: 'Connection setup is expensive, and unconstrained request concurrency can exceed database limits and consume excessive memory or processes.' },
      { question: 'How large should a connection pool be?', answer: 'Start from the database capacity, total application instance count, query latency, and target concurrency, then measure rather than using a universal number.' },
      { question: 'What happens when the pool is exhausted?', answer: 'Callers wait up to an acquisition timeout and then fail or shed work, so the service needs bounded queues and clear overload behavior.' },
    ],
    related: [
      { title: 'Backpressure', body: 'Bounds waiting when the pool is saturated.', href: '/concepts/backpressure/' },
      { title: 'Load balancing', body: 'Fleet size changes the total connection demand.', href: '/concepts/load-balancing/' },
      { title: 'Database indexing', body: 'Faster queries return connections sooner.', href: '/concepts/database-indexing/' },
    ],
  },
  {
    slug: 'concepts/cache-eviction-policies',
    term: 'Cache eviction policies',
    title: 'Cache Eviction Policies - ExtraBrain',
    h1: 'How do cache eviction policies work?',
    description: 'Cache eviction policies choose which entries to remove under a memory limit using recency, frequency, age, size, and workload cost.',
    definition: 'A cache eviction policy decides which entries leave when the cache reaches its capacity. LRU favors recently accessed data, LFU favors frequently accessed data, and TTL-based expiration removes data by age, but real workloads often combine several signals.',
    details: [
      { title: 'LRU', body: 'Least recently used eviction works well when recent access predicts near-future reuse, but one large scan can displace a valuable working set.' },
      { title: 'LFU', body: 'Least frequently used eviction protects repeatedly popular entries but needs aging so old popularity does not persist forever.' },
      { title: 'TTL and freshness', body: 'Time-to-live limits staleness and memory duration, but synchronized expiry can trigger a burst of backend reads.' },
      { title: 'Admission matters', body: 'Rejecting low-value entries before insertion can outperform choosing what to evict after every miss is admitted.' },
    ],
    practice: [
      { title: 'Run a workload trace', body: 'Step through a scan and a hot key on a visible cache timeline while ExtraBrain records why LRU, LFU, or admission control wins.' },
      { title: 'Use a private Mac study loop', body: 'Local Parakeet transcription, local session history, and optional on-device AI reduce external exposure of production-like access patterns.' },
      { title: 'Compare policies flexibly', body: 'Bring your own provider to analyze hit-rate tradeoffs. ExtraBrain has a free core and works as a meeting copilot for collaborative cache reviews.' },
    ],
    faq: [
      { question: 'What is the difference between LRU and LFU?', answer: 'LRU evicts the entry not used for the longest time, while LFU evicts the entry with the lowest access frequency.' },
      { question: 'Is TTL an eviction policy?', answer: 'TTL is an expiration rule based on age. It often works alongside a capacity policy such as LRU or LFU.' },
      { question: 'What is cache admission control?', answer: 'It decides whether a missed item is valuable enough to enter the cache, preventing one-time scans from displacing a useful working set.' },
    ],
    related: [
      { title: 'Caching', body: 'The broader latency and freshness tradeoff.', href: '/concepts/caching/' },
      { title: 'Hash maps', body: 'Provide fast key lookup inside many caches.', href: '/concepts/hash-maps/' },
      { title: 'Consistent hashing', body: 'Distributes keys across cache nodes.', href: '/concepts/consistent-hashing/' },
    ],
  },
  {
    slug: 'concepts/webhooks',
    term: 'Webhooks',
    title: 'Webhooks in System Design - ExtraBrain',
    h1: 'How do webhooks work?',
    description: 'Webhooks deliver event notifications to subscriber endpoints with signatures, retries, deduplication, ordering, and replay controls.',
    definition: 'A webhook is an outbound HTTP request sent when an event occurs, allowing a subscriber to react without polling. A production webhook system needs authentication, durable delivery, bounded retries, duplicate handling, and tools to inspect or replay failures.',
    details: [
      { title: 'Sign every delivery', body: 'A keyed signature over the raw request body and timestamp lets the receiver verify origin and reject tampered or replayed payloads.' },
      { title: 'Deliver asynchronously', body: 'Persist the event before delivery and use workers so a slow subscriber does not delay the source transaction.' },
      { title: 'Expect duplicates', body: 'Network ambiguity and retry-after-timeout behavior make at-least-once delivery common, so each event needs a stable identifier.' },
      { title: 'Operate the pipeline', body: 'Per-endpoint backoff, dead-letter storage, delivery logs, rotation controls, and manual replay make failures recoverable.' },
    ],
    practice: [
      { title: 'Design the retry path', body: 'Map event creation, worker delivery, timeout, and replay while ExtraBrain connects the visible flow to your explanation of duplicates.' },
      { title: 'Keep integration details local', body: 'The Mac-native app supports local transcription and local history for private practice with realistic endpoint and payload examples.' },
      { title: 'Audit security choices', body: 'Use compatible on-device AI or a provider you control to challenge signing and replay defenses. The free core meeting copilot captures a peer threat-model session.' },
    ],
    faq: [
      { question: 'How should webhook receivers verify a request?', answer: 'Verify a timestamped signature over the exact raw body with a shared secret, use constant-time comparison, and reject timestamps outside a replay window.' },
      { question: 'Why do webhooks need idempotency?', answer: 'A sender may retry after a timeout even when the first request succeeded, so receivers must safely recognize the same event again.' },
      { question: 'How should failed webhooks be retried?', answer: 'Retry transient failures with exponential backoff and jitter, cap the attempt window, and move exhausted deliveries to inspectable dead-letter storage.' },
    ],
    related: [
      { title: 'Retry with exponential backoff', body: 'Spaces failed delivery attempts safely.', href: '/concepts/retry-with-exponential-backoff/' },
      { title: 'Idempotency', body: 'Makes duplicate event delivery safe.', href: '/concepts/idempotency/' },
      { title: 'Message queues', body: 'Decouple durable events from delivery workers.', href: '/concepts/message-queues/' },
    ],
  },
];
