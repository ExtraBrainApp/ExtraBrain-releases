...
const articleBySlug: Record<string, string> = {'ai-interview-assistant': '## How to use an AI interview assistant well\n\nAn AI interview assistant is most useful when it helps you stay organized, not when it tries to replace your judgment. Before an interview, prepare the job description, your resume, project notes, and a few examples you can explain honestly. During a permitted session, ExtraBrain can follow the transcript and visible screen context so you can keep track of the actual question, constraints, and follow-ups.\n\nUse the assistant to structure your thinking: clarify the question, identify the strongest example, outline tradeoffs, and prepare a concise answer. For technical interviews, that can mean restating constraints, naming edge cases, and explaining complexity. For behavioral interviews, it can mean turning a rough memory into a clear situation, action, result, and lesson.\n\nAfter the call, review the transcript. Mark answers that were vague, too long, or missing evidence. That review loop is where the tool becomes valuable: each interview leaves behind better notes for the next one.', 'ai-interview-copilot': '## What a good interview copilot should do\n\nA useful interview copilot should support the conversation without distracting from it. In practice, that means it should listen to the live discussion, preserve important context, and help you organize a response when the interviewer asks a difficult follow-up. ExtraBrain is built around that workflow: transcript, screen context, and private notes stay close to the interview instead of being scattered across tabs.\n\nThe best way to prepare is to decide what support you need before the call. For a coding screen, prepare problem-solving steps, edge-case reminders, and project details. For a system design discussion, prepare requirements, bottlenecks, tradeoffs, and examples from past work. For behavioral questions, prepare stories that prove ownership, collaboration, conflict resolution, and learning.\n\nUse the copilot only where live assistance is allowed. When it is not allowed, use the same workflow for mock interviews and post-interview debriefs. The goal is stronger thinking, not hidden shortcuts.', 'ai-interview-answer-generator': '## Generating better interview answers from your own experience\n\nAn interview answer generator is only helpful if the answer still sounds like you and stays grounded in real experience. Generic answers are easy to spot. Strong answers include context, constraints, decisions, tradeoffs, and measurable outcomes. ExtraBrain helps by keeping your resume, job description, transcript, and notes available as context, so suggested answer structures start from material you actually know.\n\nFor behavioral questions, begin with the story you want to tell, then shape it into a clear arc: situation, responsibility, action, result, and reflection. For technical questions, use generated structure to explain assumptions, options, complexity, and tests. For product or strategy prompts, use it to organize customer, business, and execution tradeoffs.\n\nThe final step is editing. Remove claims you cannot defend, add concrete details, and practice aloud until the answer feels natural. A good generated answer should become a better version of your own thinking.', 'best-ai-interview-assistant': '## Choosing the best AI interview assistant\n\nThe best AI interview assistant is not the one that produces the longest answer. It is the one that helps you understand the question, organize your response, and review what happened afterward. When comparing tools, look for live transcription, screen-aware context, privacy controls, provider choice, and useful session history. These features matter because interviews are dynamic: the prompt changes, follow-ups appear, and your best example may depend on the exact wording.\n\nExtraBrain is designed for candidates who want a private desktop workflow. You can prepare with your real resume and notes, follow allowed live conversations, and review transcripts after practice sessions. That makes it useful before, during, and after the interview instead of only at the moment an answer is needed.\n\nA good evaluation test is simple: run a mock interview, save the transcript, and ask whether the tool helped you improve the next attempt. If it did, it is doing more than generating text.', 'technical-interview-ai-assistant': '## Using AI during technical interview preparation\n\nTechnical interviews test more than whether you know the final answer. Interviewers want to see how you clarify constraints, choose data structures, reason about tradeoffs, test edge cases, and communicate while solving. A technical interview assistant should help with that process, not just provide a solution. ExtraBrain can keep the prompt, transcript, visible code, and notes together so you can review your reasoning after the session.\n\nBefore practice, prepare a small set of patterns: arrays, hash maps, trees, graphs, dynamic programming, APIs, databases, and system design basics. During a mock interview, say your assumptions aloud and use the assistant to keep track of missed constraints or follow-up questions. Afterward, review where you got stuck: problem understanding, algorithm choice, implementation, testing, or explanation.\n\nThat review is the real advantage. One saved transcript can reveal repeated habits that a solved problem alone would hide.', 'responsible-ai-interview-assistant': '## Responsible AI use in interview preparation\n\nResponsible AI interview support starts with a simple rule: follow the rules of the interviewer, employer, school, or platform. If live assistance is not allowed, use AI before the interview for practice and after the interview for review. If live assistance is allowed, keep answers honest and based on your own experience. ExtraBrain is most useful when it helps you organize real context, not invent accomplishments.\n\nA responsible workflow begins before the call. Prepare your resume, project notes, target role, and examples you can explain in detail. During practice, use transcripts to notice unclear answers or weak evidence. After the session, improve the answer yourself so you can deliver it naturally next time.\n\nPrivacy matters too. Interviews often involve compensation goals, source code, company names, or personal career history. Check what is visible, what is transcribed, and which provider is selected before using any assistant in a sensitive setting.', 'use-cases/product-management-interviews': '## Preparing for product management interviews\n\nProduct management interviews reward structured thinking, customer empathy, prioritization, and clear tradeoffs. The hardest part is often not knowing a framework; it is applying the right structure to the actual prompt. ExtraBrain can help you keep the question, transcript, and notes visible while you organize your answer around users, goals, constraints, metrics, and execution risks.\n\nFor product sense questions, practice naming the target user, pain point, success metric, and alternatives. For strategy questions, prepare market assumptions, business model tradeoffs, and possible risks. For execution questions, be ready to discuss prioritization, stakeholder alignment, rollout plans, and measurement. Behavioral rounds should connect your examples to product judgment: ambiguity, influence, conflict, and learning.\n\nAfter each mock, review whether your answer had a clear recommendation. If the transcript shows a lot of ideas but no decision, practice synthesizing sooner and stating tradeoffs more directly.', 'use-cases/data-science-interviews': '## Preparing for data science interviews\n\nData science interviews combine statistics, modeling, experimentation, product sense, and communication. A strong candidate can explain not only the model or metric, but also why it fits the business problem. ExtraBrain can help during practice by keeping the prompt, transcript, charts, notes, and assumptions together for later review.\n\nPrepare for several interview modes. For statistics, review sampling, confidence, bias, experiment design, and interpretation. For modeling, practice feature choices, validation, leakage, error analysis, and tradeoffs between accuracy and usefulness. For product questions, connect metrics to user behavior and business outcomes. For project deep dives, explain your role, data quality issues, decisions, results, and limitations.\n\nAfter a mock interview, look for missing explanations. Did you define the metric? Did you state assumptions? Did you explain why the method was appropriate? Those gaps are easier to fix when the session is saved.', 'use-cases/technical-phone-screens': '## Handling technical phone screens with more structure\n\nA technical phone screen moves quickly. You may need to understand a prompt, ask clarifying questions, explain an approach, code or reason aloud, and respond to hints in a short window. ExtraBrain can help in practice by capturing the transcript and visible prompt so you can see exactly where your reasoning became unclear.\n\nPrepare a repeatable flow. Restate the problem, confirm inputs and outputs, ask about constraints, give a simple example, propose a brute-force approach, then improve it. If coding is involved, speak through edge cases and tests before rushing into implementation. If the screen is more conversational, prepare project examples and technical decisions that prove depth.\n\nAfterward, review the call like a debugging session. Was the issue problem comprehension, algorithm choice, syntax, complexity, or communication? Fixing the right category is much more effective than solving random extra problems.', 'use-cases/frontend-interviews': '## Preparing for frontend interviews\n\nFrontend interviews often mix JavaScript fundamentals, UI architecture, accessibility, performance, debugging, and product judgment. A strong answer shows that you can build interfaces people can actually use. ExtraBrain can support practice by keeping the prompt, screen context, transcript, and notes together while you explain component decisions or debug behavior.\n\nPrepare beyond framework trivia. Review state management, rendering behavior, forms, async data, error states, responsive layouts, keyboard access, semantic HTML, and performance bottlenecks. For system design-style frontend rounds, practice discussing component boundaries, data flow, caching, loading states, and observability. For portfolio or resume questions, be ready to explain tradeoffs you made in real projects.\n\nAfter a mock, review whether your answer included users and constraints. Frontend work is not only code; it is the connection between product behavior, visual structure, accessibility, and maintainability.', 'use-cases/backend-interviews': '## Preparing for backend interviews\n\nBackend interviews test APIs, data modeling, reliability, scalability, debugging, and tradeoff thinking. The best candidates explain not only what they would build, but why the design fits the constraints. ExtraBrain can help by capturing the prompt, diagrams, transcript, and notes so you can review whether your explanation was complete.\n\nPractice core backend areas: request flows, database schemas, indexes, transactions, queues, caching, rate limits, idempotency, observability, and failure handling. For system design prompts, start with requirements and scale assumptions before jumping into services. For project deep dives, prepare incidents, migrations, architecture decisions, and lessons learned.\n\nAfter each practice session, review where your answer became vague. Did you define consistency needs? Did you discuss retries and failure modes? Did you justify storage choices? Backend interviews reward clear tradeoffs more than memorized architecture diagrams.', 'best-interview-prep-tools': '## Building an interview prep tool stack\n\nThe best interview prep tools work together. You need question banks for breadth, mock interviews for pressure, technical practice for skill, resume review for clarity, and notes or transcripts for improvement. ExtraBrain fits into the stack as the private context layer: it helps you practice with your real resume, job description, prompts, and previous answers instead of starting over every time.\n\nChoose tools based on the interview format. Coding interviews need problem practice and explanation review. Case interviews need structure, math, and synthesis. Behavioral interviews need story preparation and follow-up practice. Internship interviews need project walkthroughs and motivation answers. A single generic tool will not cover all of those equally well.\n\nThe key is the review loop. After each mock, save what happened, identify one weak area, and practice it again. Tools that help you improve the next attempt are more valuable than tools that only generate more questions.', 'software-engineering-interview-preparation': '## A practical software engineering interview plan\n\nSoftware engineering interview preparation should combine coding practice, system design, project depth, and communication. Start by mapping the loop: recruiter screen, coding assessment, technical phone screen, system design, behavioral round, and final discussion. Each round tests a different skill, so one study method is not enough.\n\nFor coding, practice patterns and explanation together. Restate the problem, name constraints, compare approaches, analyze complexity, and test edge cases. For system design, begin with requirements, APIs, data model, bottlenecks, and reliability. For behavioral questions, prepare stories about ownership, conflict, ambiguity, debugging, and technical decisions.\n\nExtraBrain can help you capture prompts, visible code, notes, and transcripts from mock sessions. Review those records to find recurring problems: jumping into code too early, forgetting tests, weak complexity analysis, or vague project details. Preparation improves fastest when every practice session leaves a specific next task.', 'data-analyst-interview-preparation': '## A practical data analyst interview plan\n\nData analyst interviews test whether you can turn messy business questions into clear analysis. Prepare SQL, metrics, dashboards, statistics, and communication together. A strong answer explains the business goal, data needed, assumptions, query or method, validation checks, and recommendation.\n\nStart with SQL fundamentals: joins, grouping, windows, date logic, nulls, and query explanation. Then practice metric cases such as conversion drops, retention changes, revenue movement, or dashboard design. For statistics, focus on interpretation: sampling, confidence, bias, correlation, causation, and experiment results. For resume questions, prepare analyses that changed a decision, not just reports you built.\n\nExtraBrain can help during mock interviews by saving the prompt, transcript, and visible work. After each session, review whether you clarified the metric, checked data quality, and explained the business impact. Those habits separate a good analyst from someone who only knows syntax.', 'consulting-case-interview-prep': '## A practical consulting case prep plan\n\nConsulting case interviews test structure, math, business judgment, and synthesis under pressure. The goal is not to recite a framework. The goal is to understand the client objective, build a useful structure, test the most important drivers, and communicate a recommendation clearly.\n\nPrepare by practicing different case types: profitability, market entry, growth, pricing, operations, and customer experience. For each case, start with clarifying questions and a tailored issue tree. During analysis, state assumptions, keep units visible, sanity-check numbers, and explain why each calculation matters. When exhibits appear, summarize the insight before diving into details.\n\nExtraBrain can support practice by saving case prompts, transcripts, and feedback. After each mock, review where you lost structure: opening, math, chart reading, prioritization, or final synthesis. Strong case prep is built through focused debriefs, not just doing more cases.', 'internship-interview-preparation': '## A practical internship interview plan\n\nInternship interviews are often about potential. Interviewers want to know what you have learned, how you think, why you want the role, and whether you can communicate clearly. Start with the job description and map each requirement to a class project, personal project, activity, part-time job, or team experience.\n\nPrepare the common questions first: tell me about yourself, why this company, why this internship, walk me through a project, and tell me about a time you worked on a team. For technical internships, review the fundamentals named in the posting. For business or analytical internships, prepare examples that show problem solving, organization, and curiosity.\n\nExtraBrain can help you practice aloud and save mock interview transcripts. Review whether your answers were specific enough. If you only said you are hardworking or passionate, replace that with evidence: what you built, what changed, what you learned, and what you want to try next.', 'resume-advice-for-job-interviews': '## Turning your resume into interview answers\n\nYour resume is not only a document for applications. It becomes the interviewer’s question map. Every bullet can lead to a follow-up about your role, decisions, tools, metrics, teammates, constraints, or lessons learned. Good interview preparation means being ready to explain the story behind each important line.\n\nStart by annotating your resume. For each bullet, write the problem, your responsibility, the action you took, the result, and what you would do differently now. Then compare those stories with the job description. Choose the examples that best prove the skills the role needs. If a metric appears on the resume, make sure you can explain the baseline, calculation, and your contribution.\n\nExtraBrain can help you practice resume walkthroughs with the job description and your notes in context. After a mock, review where you sounded vague or overclaimed. The strongest resume answers are specific, honest, and easy to defend.', 'platforms/coderpad': '## Preparing for CoderPad interviews with ExtraBrain\n\nCoderPad interviews usually combine live coding, explanation, debugging, and collaboration. The interviewer can see how you think, not only whether the final code runs. Prepare by practicing the full flow: restate the prompt, clarify constraints, propose an approach, code in small steps, test edge cases, and explain tradeoffs as you go.\n\nExtraBrain can support permitted practice by capturing the prompt, transcript, and visible coding context for review. Use it after mock sessions to see where the conversation drifted: did you ask enough questions, miss an edge case, or stop explaining while coding? For real interviews, follow the rules given by the interviewer and platform.\n\nBefore the call, test your audio, browser, keyboard shortcuts, and screen layout. Keep notes concise: common complexity reminders, test-case habits, and project details. CoderPad rewards calm communication under pressure, so practice speaking while editing instead of solving silently.', 'platforms/hackerrank': '## Preparing for HackerRank interviews and assessments\n\nHackerRank can be used for timed coding assessments, live technical interviews, and skill screens. The format matters because a timed test requires different habits than a collaborative interview. For assessments, practice reading carefully, managing time, and writing tests quickly. For live sessions, practice explaining your reasoning while editing code.\n\nExtraBrain can help in practice by saving prompts, transcripts, and review notes after mock sessions. Use those records to identify repeated issues: misunderstanding constraints, skipping edge cases, overcomplicating the solution, or running out of time. If you use any assistant in a real assessment, follow the platform and employer rules exactly.\n\nPrepare a short checklist before each session: input size, edge cases, complexity, test examples, and final explanation. HackerRank success often comes from disciplined execution, not just knowing algorithms.', 'platforms/codesignal': '## Preparing for CodeSignal interviews and screens\n\nCodeSignal assessments often emphasize speed, accuracy, and careful reading. Whether you are taking a certified assessment or joining a live technical screen, the core skill is the same: understand the prompt, choose an efficient approach, and avoid preventable mistakes.\n\nPractice under timed conditions before the real session. Review arrays, strings, hash maps, sorting, graphs, and dynamic programming patterns, but also practice test design and debugging. ExtraBrain can help during mock preparation by keeping prompts, transcripts, and notes together so you can review where time was lost. In real assessments, follow CodeSignal and employer rules for allowed tools.\n\nAfter practice, sort mistakes into categories: comprehension, algorithm choice, implementation, edge cases, or time management. That diagnosis gives you a better study plan than simply solving more random problems.', 'platforms/zoom': '## Using ExtraBrain with Zoom interviews and meetings\n\nZoom interviews and meetings can move quickly, especially when screen sharing, technical prompts, or multi-person discussions are involved. ExtraBrain can run alongside Zoom as a desktop assistant, helping you capture transcript and screen context for preparation, permitted live use, and review.\n\nBefore a Zoom interview, test audio, permissions, screen layout, and any materials you plan to reference. Prepare a small set of notes: resume stories, project details, questions for the interviewer, and reminders about the role. If live AI assistance is not allowed, use ExtraBrain for mock interviews and post-call debriefs instead.\n\nAfter a session, review the transcript while the conversation is still fresh. Look for missed questions, vague answers, weak examples, or follow-ups you want to send. The value of a Zoom copilot is not only during the call; it is also the record it leaves for improvement.', 'platforms/google-meet': '## Using ExtraBrain with Google Meet interviews\n\nGoogle Meet is common for remote interviews, recruiter screens, technical conversations, and team calls. The challenge is staying present while tracking the question, your notes, and any shared screen content. ExtraBrain can help by keeping transcript and context available in a separate desktop workflow.\n\nPrepare before the meeting. Check microphone permissions, browser setup, screen layout, and whether you will need to share anything. For interviews, keep resume stories, project examples, role notes, and questions for the interviewer nearby. For technical calls, prepare reminders about constraints, tests, and tradeoffs. Always follow the rules for AI assistance in the specific interview or meeting.\n\nAfter the Google Meet session, use the transcript to write a short debrief. What was asked? What did you answer well? What should be clearer next time? That review turns one call into better preparation for the next.', 'platforms/microsoft-teams': '## Using ExtraBrain with Microsoft Teams interviews\n\nMicrosoft Teams is often used for enterprise interviews, panel calls, technical discussions, and cross-functional meetings. These sessions may include multiple speakers, screen sharing, and detailed follow-ups. ExtraBrain can support preparation and allowed live use by capturing transcript and screen context in a private desktop workflow.\n\nBefore a Teams interview, test audio, permissions, calendar links, and your screen layout. Prepare concise notes: job requirements, resume examples, project details, and questions to ask. For panels, write down the role of each interviewer if you know it, because different people may care about different evidence.\n\nAfterward, review the transcript and create a follow-up plan. Identify unclear answers, names or topics to reference in thank-you notes, and examples to improve before the next round. Teams calls often contain useful details that are easy to forget without a saved record.', 'platforms/leetcode': '## Preparing for LeetCode-style interviews\n\nLeetCode practice is useful when it builds pattern recognition and explanation skill together. The goal is not to memorize hundreds of answers. The goal is to recognize constraints, choose a data structure, explain complexity, and test edge cases under interview pressure.\n\nUse ExtraBrain during mock practice to capture the prompt, transcript, and your reasoning. After solving, review whether you clarified the problem, considered a brute-force approach, explained the optimized approach, and tested important cases. If you got stuck, label the reason: pattern gap, implementation issue, recursion confusion, time pressure, or communication.\n\nFor real interviews or assessments, follow the platform and employer rules. Outside the live session, build a focused study loop: solve one problem, explain it aloud, review the transcript, and write the lesson in your own words. That is more effective than passive grinding.', 'interview-guides/openai': '## Preparing for OpenAI interviews\n\nOpenAI interviews often emphasize technical depth, clear reasoning, and thoughtful discussion of complex systems or research-adjacent problems. Prepare by studying the role closely and mapping each requirement to a project, technical decision, or example you can explain in detail.\n\nFor engineering roles, practice coding, system design, debugging, and project deep dives. Be ready to discuss tradeoffs, reliability, model or product constraints, and how you make decisions under uncertainty. For research or applied AI roles, prepare to explain methods, evaluation choices, limitations, and why your work mattered. Behavioral questions should show ownership, curiosity, collaboration, and judgment.\n\nExtraBrain can help during mock preparation by saving transcripts, prompts, and notes. Review whether your answers were specific enough for a high-depth interview. OpenAI preparation should feel less like memorizing answers and more like sharpening how you explain hard work clearly.', 'interview-guides/anthropic': '## Preparing for Anthropic interviews\n\nAnthropic interviews may explore technical skill, product judgment, research thinking, communication, and alignment with the company’s safety-oriented mission. Start by understanding the role and preparing examples that show careful reasoning, collaboration, and responsible decision-making.\n\nFor technical rounds, practice explaining systems, code, experiments, or model-related work with clear assumptions and limitations. For behavioral rounds, prepare stories about ambiguity, feedback, ethical tradeoffs, and cross-functional collaboration. If your background includes AI or infrastructure work, be ready to discuss evaluation, reliability, failure modes, and impact.\n\nExtraBrain can support practice by keeping your notes, prompts, and transcripts together. After each mock, review whether your answer showed depth and humility. Anthropic-style preparation benefits from being precise about what you know, honest about uncertainty, and clear about how you reason through difficult problems.', 'interview-guides/google': '## Preparing for Google interviews\n\nGoogle interviews often focus on problem solving, technical fundamentals, communication, and role-related experience. For software roles, expect coding, data structures, algorithms, system design at higher levels, and behavioral examples. For other roles, prepare structured examples that show analytical thinking, collaboration, and impact.\n\nBuild a preparation plan around the expected loop. Practice coding aloud with constraints and tests. Prepare project stories that explain your role, tradeoffs, metrics, and lessons learned. For behavioral questions, focus on examples of leadership, ambiguity, conflict, and learning. Google interviewers often care about how you think, not only the final answer.\n\nExtraBrain can help you run mock interviews with saved transcripts and visible prompts. Review whether you clarified the question, communicated clearly, and supported claims with evidence. A strong Google interview answer is structured, specific, and easy to follow.', 'interview-guides/meta': '## Preparing for Meta interviews\n\nMeta interviews often emphasize speed, clarity, product impact, and technical execution. Software candidates should prepare coding patterns, system design, and behavioral stories that show ownership and results. Product, data, and other roles should prepare structured examples tied to users, metrics, tradeoffs, and decision-making.\n\nFor coding rounds, practice explaining the approach quickly and testing edge cases. For system design, focus on requirements, scale, APIs, data models, and reliability. For behavioral rounds, prepare stories about moving fast, resolving conflict, learning from mistakes, and influencing without perfect information.\n\nExtraBrain can help during practice by capturing prompts and transcripts so you can review pacing and clarity. Meta preparation benefits from concise answers with strong evidence. If the transcript shows rambling or delayed decisions, practice shorter summaries and clearer recommendations before the next mock.', 'interview-guides/amazon': '## Preparing for Amazon interviews\n\nAmazon interviews are strongly tied to Leadership Principles, technical depth, and practical ownership. Prepare stories that show customer obsession, bias for action, ownership, high standards, learning, and delivering results. Each story should include context, your specific role, decisions made, measurable outcome, and what you learned.\n\nFor technical roles, combine coding or system design preparation with behavioral depth. Interviewers may probe deeply into project details, tradeoffs, failures, and metrics. Do not prepare vague stories; prepare examples you can defend through several follow-up questions.\n\nExtraBrain can help you practice by saving mock transcripts and highlighting where an answer lacked evidence. After each session, review whether your story matched a Leadership Principle and whether the result was clear. Amazon preparation is strongest when every answer is specific, measurable, and grounded in ownership.', 'interview-guides/microsoft': '## Preparing for Microsoft interviews\n\nMicrosoft interviews often evaluate technical fundamentals, collaboration, product thinking, and growth mindset. Prepare to explain how you solve problems, work with others, learn from feedback, and make tradeoffs. For engineering roles, practice coding, debugging, system design, and project deep dives.\n\nA good Microsoft interview answer is clear and collaborative. In coding rounds, talk through constraints, approach, tests, and alternatives. In behavioral rounds, prepare examples about teamwork, ambiguity, conflict, customer impact, and learning. For product or program roles, connect decisions to users, business goals, and execution details.\n\nExtraBrain can support mock preparation by saving transcripts, prompts, and notes. Review whether you sounded collaborative and specific. If an answer was technically correct but hard to follow, practice explaining the same idea in a simpler, more structured way.', 'interview-guides/apple': '## Preparing for Apple interviews\n\nApple interviews often value product quality, craft, privacy, collaboration, and technical excellence. Prepare examples that show attention to detail, strong judgment, and the ability to build polished work under constraints. Technical candidates should be ready for coding, systems, debugging, and deep project discussion.\n\nFor project walkthroughs, explain the user problem, design or technical tradeoffs, quality bar, and outcome. For behavioral questions, prepare stories about collaboration, disagreement, ownership, and learning. If the role touches user experience, privacy, hardware, software, or platform constraints, be ready to explain why your choices served the end user.\n\nExtraBrain can help you practice and review transcripts after mock interviews. Look for places where your answer was too generic. Apple preparation benefits from concrete details about craft, constraints, and the reasoning behind decisions.', 'interview-guides/stripe': '## Preparing for Stripe interviews\n\nStripe interviews often emphasize clarity, product sense, technical depth, and practical problem solving. Candidates should prepare to discuss systems, APIs, reliability, user experience, business impact, and written or verbal communication. The best answers are precise and grounded in real work.\n\nFor engineering roles, practice coding, debugging, system design, and project deep dives. Be ready to discuss tradeoffs, operational concerns, developer experience, and how you handle ambiguity. For product or business roles, prepare examples that connect customer pain, metrics, constraints, and execution.\n\nExtraBrain can help during mock preparation by saving prompts, transcripts, and notes. Review whether you explained the problem clearly before jumping into the solution. Stripe preparation rewards crisp thinking, so practice concise summaries, explicit assumptions, and clear recommendations.', 'interview-guides/databricks': '## Preparing for Databricks interviews\n\nDatabricks interviews may involve distributed systems, data infrastructure, performance, product thinking, and technical depth. Prepare examples that show how you reason about scale, reliability, data, and engineering tradeoffs. For technical roles, coding and system design practice should be paired with deep project explanations.\n\nReview topics such as data processing, storage, APIs, concurrency, observability, and performance bottlenecks where relevant to the role. For project deep dives, be ready to explain architecture, constraints, incidents, metrics, and decisions. Behavioral answers should show ownership, collaboration, and learning in complex technical environments.\n\nExtraBrain can help you run focused mock sessions and preserve the transcript for review. After practice, check whether you made tradeoffs explicit. Databricks preparation is strongest when you can explain not only what you built, but why it worked at scale.', 'interview-guides/netflix': '## Preparing for Netflix interviews\n\nNetflix interviews often emphasize judgment, ownership, communication, and impact. The company is known for a culture that values context, responsibility, and candid discussion, so prepare examples that show how you make decisions, handle ambiguity, and deliver meaningful outcomes.\n\nFor technical roles, practice coding, architecture, reliability, and project deep dives. Be ready to discuss tradeoffs, failures, and lessons learned. For product, data, or business roles, prepare examples that connect user value, metrics, strategy, and execution. Behavioral stories should be specific and honest, not polished slogans.\n\nExtraBrain can help you practice by saving transcripts and notes from mock interviews. Review whether your answers showed judgment and ownership. Netflix preparation benefits from clear stories about real decisions: what context you had, what you chose, what happened, and what you learned.'};

const secondBrainArticle = `## What an AI second brain means for live work

Most second brain systems are built around notes, documents, tags, and databases. That can be useful, but interviews and meetings create a different kind of knowledge. The useful context is often spoken once, shown briefly on screen, or scattered across a transcript, prompt, whiteboard, resume, project note, and follow-up question.

ExtraBrain focuses on that live context. It can capture transcripts, visible screen context, notes, screenshots, and analysis so a conversation leaves behind something you can review. The goal is not to replace a full personal knowledge management system. The goal is to make important sessions easier to remember, search, summarize, and act on.

For interview prep, that means your mock sessions become reusable evidence. You can see which stories sounded vague, which technical explanations missed constraints, and which follow-up questions deserve practice. For meetings, it means decisions, risks, owners, and unresolved questions do not disappear after the call.

## Capture first, organize second

A second brain fails when maintaining it becomes the real job. Interviews and meetings already demand attention, so the capture workflow needs to be light. Start the session, confirm what should be recorded or analyzed, and let the transcript and screen context become the raw material.

After the session, organize only what matters. Save the decision, the action item, the interview story, the technical lesson, or the customer quote. Delete or ignore noise. This keeps your memory system practical instead of turning it into another archive you never revisit.

## Where ExtraBrain fits

ExtraBrain fits between a meeting assistant, an interview copilot, and a private memory workspace. It is useful when the source material comes from a live conversation and the next step depends on remembering that conversation accurately.

Use it before a session to prepare role notes, project examples, and prompts. Use it during permitted sessions to follow transcript and screen context. Use it afterward to summarize, review, redact, and turn the session into better next steps.

## Keep sensitive memory under control

An AI second brain can contain private material: compensation goals, customer names, internal tools, source code, hiring feedback, business strategy, and personal reflections. That is why data flow matters.

ExtraBrain is local-first by design. With local Parakeet transcription plus local Gemma 4 where installed and compatible, a local posture is possible. If you choose external AI or transcription providers, selected prompts, transcript text, screenshots, audio, or context may leave your device according to that provider setup.

Before a sensitive session, check what is visible, what is being transcribed, and which provider is selected. Use ExtraBrain only where interview, workplace, school, meeting, and platform rules allow transcription, screenshots, notes, or AI assistance.`;

const withSeoArticleContent = (page: MarketingPage): MarketingPage => ({
  ...page,
  sections: [
    ...page.sections.slice(0, 1),
    {
      title: 'Practical guide',
      bodyMarkdown: page.slug === "ai-second-brain" ? secondBrainArticle : articleBySlug[page.slug],
    },
